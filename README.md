...
`run.sh` は Linux と Windows（Git Bash / MSYS2）の両方で動作します。`stop` / `restart` の対象プロセスは、Linux では `pgrep`（無い場合は `ps`）、Windows では PowerShell でコマンドラインに `multimodal_retriever` を含む `python`/`uv` プロセスを検索して特定します。Windows には `pgrep` / `pkill` は不要です。
...
sequenceDiagram
    ...
    participant EV as WorkflowAgenticEvents
...
- `generate_final_answer`: 選別済み evidence で最終回答を生成します。`answerable`（true=完全回答可、false=情報不足）を必ず指定します。
...
- 検索 evidence が 0 件でもアップロード画像があれば VLM で回答を試行します。
- 検索結果がある場合も、アップロード画像を VLM 入力の 1 枚目、参照画像を 2 枚目以降に渡します。
- 回答冒頭には `ユーザーがアップロードした画像` と検索結果のファイル名を併記します。

質問なしで画像だけを入力した場合は、画像ベクトル検索のみを実行し、類似画像を `参照した画像` に表示します（Controller / VLM 呼び出しなし）。
...
flowchart TD
    ...
    Ctrl --> ParseOK{parse OK?}
    ParseOK -->|No| ParseErr{連続エラー上限?}
    ParseErr -->|Yes| MaxErr[Controllerエラー終了]
    ParseErr -->|No| Loop
    ...
    Stale -->|No| Loop
    Action -->|select_evidence| Select[evidence選別]
    Select --> Verifier1{answerable false?}
    Verifier1 -->|Yes| FVG1[FinalizeVerifier]
    FVG1 -->|new_leads| Hold1[確定保留]
    Hold1 --> Loop
    FVG1 -->|no leads| Loop
    Verifier1 -->|No| Loop
    Action -->|generate_final_answer| GenCheck{selected or uploaded?}
    GenCheck -->|No| Loop
    GenCheck -->|Yes| Verifier2{answerable false?}
    Verifier2 -->|Yes| FVG2[FinalizeVerifier]
    FVG2 -->|new_leads| Hold2[確定保留]
    Hold2 --> Loop
    FVG2 -->|no leads| VLM[VLM回答生成]
    Verifier2 -->|No| VLM
    VLM --> End
    Action -->|invalid| Loop
    Insufficient --> End
...
ループを return せずに max_steps 回終えると、最大ステップ終了となります。
...
sequenceDiagram
    participant UI as GradioUI
    participant EV as ReactAgenticEvents
    participant PL as ReactPipeline
    participant Ctrl as ControllerLLM
    participant Reg as ReactToolRegistry
    participant SS as SearchService
    participant Ver as FinalizeVerifier
    participant VLM as AnswerVLM

    UI->>EV: run_react_agentic_rag
    EV->>PL: run_stream(answer_generator)
    loop each step until return
        PL->>Ctrl: _call_controller
        alt parse error
            Ctrl-->>PL: error
            PL->>PL: consecutive_errors check
        else search action
            ...
            Ctrl-->>PL: thought, action, action_input
            PL->>Reg: _select_evidence
            opt answerable false and new_leads
                PL->>Ver: _run_finalize_verifier
                Ver-->>PL: new_leads
                PL->>PL: 確定保留 continue
            end
            ...
            Ctrl-->>PL: thought, action, action_input
            opt answerable false and new_leads
                PL->>Ver: verifier gate
                Ver-->>PL: new_leads
                PL->>PL: continue loop
            end
            PL->>EV: answer_generator
            EV->>VLM: _generate_answer_with_vlm
            VLM-->>EV: answer
            EV-->>PL: answer
            PL-->>UI: yield完了
        end
...
- `画像詳細`: 選択画像のファイル名、イメージID、コサイン類似度または全文検索スコア、キャプションを表示します。
